...
// ─── Film Post-Production ──────────────────────────────────────────────────
    // ADR, Foley, Score Cues, Mix Console — all backed by real DB tables.
    // Procedure names match SoundEffects.tsx consumer exactly.
    filmPost: router({
      // ── Mix Console ──────────────────────────────────────────────────────────
      ...
      resetMixSettings: protectedProcedure
        .input(z.object({ projectId: z.number() }))
        .mutation(async ({ ctx, input }) => {
          return db.upsertFilmMixSettings(input.projectId, ctx.user.id, {
            ...
            reverbRoom: "none", reverbAmount: 0, compressionRatio: 1.0, noiseReduction: false, notes: "",
          });
        }),
      ...
      exportMixSummary: protectedProcedure
        .input(z.object({ projectId: z.number() }))
        .mutation(async ({ ctx, input }) => {
          const settings = await db.getFilmMixSettings(input.projectId, ctx.user.id);
          const adr = await db.getProjectAdrTracks(input.projectId, ctx.user.id);
          const foley = await db.getProjectFoleyTracks(input.projectId, ctx.user.id);
          const score = await db.getProjectScoreCues(input.projectId, ctx.user.id);
          return {
            exportedAt: new Date().toISOString(),
            mixSettings: settings,
            trackCounts: { adr: adr.length, foley: foley.length, score: score.length },
            summary: [
              `Mix Console: Dialogue ${Math.round((settings?.dialogueBus ?? 0.85) * 100)}% | Music ${Math.round((settings?.musicBus ?? 0.70) * 100)}% | SFX ${Math.round((settings?.effectsBus ?? 0.75) * 100)}%`,
              `ADR: ${adr.length} tracks (${adr.filter(t => t.status === "approved").length} approved)`,
              `Foley: ${foley.length} tracks (${foley.filter(t => t.status === "approved").length} approved)`,
              `Score: ${score.length} cues`,
            ].join("\n"),
          };
        }),
      ...
      generateAdrSuggestions: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const suggestions = characters.slice(0, 5).map((char, i) => ({
            ...
            trackType: "adr" as const,
            notes: "Auto-suggested — review and edit before recording",
          }));
          return { suggestions };
        }),
      ...
      generateFoleySuggestions: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const defaults = [
            ...
            { name: "Door open/close", foleyType: "props" as const, description: "Interior door handling" },
            { name: "Clothing rustle", foleyType: "cloth" as const, description: "Fabric movement during action scenes" },
            { name: "Body impact", foleyType: "impacts" as const, description: "Physical contact sound effect" },
          ];
          const suggestions = defaults.slice(0, Math.min(scenes.length + 2, defaults.length)).map(s => ({
            ...s,
            notes: "Auto-suggested — verify against picture",
          }));
          return { suggestions };
        }),
      ...
      generateScoreCues: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const cues = [
            ...
            { cueNumber: "2M1", title: "Rising Action", cueType: "underscore" as const, description: "Tension build", duration: 45, notes: "Auto-suggested" },
            { cueNumber: "3M1", title: "Climax", cueType: "theme" as const, description: "Peak dramatic moment", duration: 120, notes: "Auto-suggested" },
            { cueNumber: "4M1", title: "Resolution", cueType: "underscore" as const, description: "Denouement and end credits", duration: 90, notes: "Auto-suggested" },
          ].slice(0, Math.min(scenes.length + 1, 5));
          return { cues };
        }),
    }),
});
